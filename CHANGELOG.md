...
- Remove public `password` readers from `Client` and `LftpCommand`; redact URL userinfo and `password=` fragments from exception output (issue #8). **Breaking** for callers that used `client.password`.
